...
### 1.1 Shared mutable state on JWTBearer instance creates a race condition enabling cross-user token leakage
...
### 1.2 Unhandled Pydantic ValidationError when JWT payload is missing required claims
...
### 1.3 Incomplete PyJWT exception coverage
...
### 1.4 Authorization code replay vulnerability (no single-use enforcement)
...
### 1.7 Authorization code not deleted on PKCE validation failure, enabling brute-force attacks
...
### 1.8 Response type error uses wrong OAuth error message
...
### 1.9 OAuthException never includes error_description in HTTP response
...
### 1.10 Missing WWW-Authenticate headers on 401 responses
...
### 1.11 Debug mode configurable via settings may expose stack traces in production
...
### 1.12 CORS middleware allows all origins in an authentication service
...
### 1.13 Logging full Lambda event may expose sensitive request data
...
### 1.14 Redirect URI accepted from user input without validation at the router layer
...
### 1.16 Scope check uses OR instead of AND logic
...
### 1.17 Token can be None, causing AttributeError crash
...
### 1.19 Token deleted before new tokens are generated, risking data loss on failure
...
### 1.20 Crash if `service.scopes` is None
...
### 1.21 Env file loading order gives .env highest priority, silently overriding production values
...
### 1.22 Logger initialized before env files are loaded
...
### 1.23 ErrorResponse timestamp evaluated once at class definition time, not per-instance
...
### 1.26 Critical test false positives: get_by_id and get_by_refresh_token tests never assert return values
...
### 1.27 Test false positive: Token refresh success test does not assert token issuance
...
### 1.28 Monkeypatch-based 404 tests can produce false positives
...
### 1.32 Extra fields silently ignored in request models
...
### 2.1 No timeout configured on any HTTP request to the user service
...
### 2.2 No httpx.Client reuse, preventing connection pooling
...
### 2.4 httpx.RequestError (connection/timeout failures) never caught in any method
...
### 2.6 Success log emitted before JSON parsing, producing misleading traces on failure
...
### 2.7 Redirect URI comparison without normalization
...
### 2.8 PKCE defaults not compliant with RFC 7636
...
### 2.31 Password hasher uses default (slow) Argon2 parameters in test fixtures
...
### 2.35 Create token test and fixture create token test have tautological assertions
...
### 2.40 Token refresh test does not assert token issuance
...
### 2.41 Correlation ID tests do not verify preservation
...
### 3.10 `aud` field only supports single string, not array of audiences
...
### 3.11 `expires_in` accepts zero and negative values
...
### 3.12 `token_type` is a plain str with no restriction
...
### 3.13 `service_token_lifetime` default of 30 has ambiguous units
...
### 3.16 Inconsistent naming: client_id vs client_name
...
### 3.20 Unconventional `__import__` usage instead of standard import
...
- **Concurrency:** At least two critical race conditions (JWTBearer state, refresh token reuse) that will surface under load.
...
1. **Fix the JWTBearer shared mutable state race condition** (`app/jwt_bearer.py:110,125`). This is the most impactful bug: it can cause one user to receive another user's authentication in a concurrent environment. Replace `self.decoded_token` with a local variable.

2. **Add single-use enforcement for authorization codes** (`app/models/authorization_code.py` and `app/services/auth_service.py`). Without this, an attacker who intercepts a single authorization code can replay it indefinitely to mint tokens. Add a `used` flag and atomic consume operation.

3. **Fix the refresh token reuse race condition** (`app/services/auth_service.py:269-291`). Use a conditional DynamoDB delete so the second concurrent request fails atomically instead of producing duplicate token pairs.

4. **Catch Pydantic ValidationError in JWT token parsing** (`app/jwt_bearer.py:116-117`). This unhandled exception turns every malformed JWT (missing claims) into a 500 error instead of a proper 403. Add `except ValidationError` alongside the existing exception handlers.

5. **Add SSM parameter caching and error handling** (`app/settings.py:26-48`). Every property access makes a live SSM API call with no caching. Use `@cached_property` and add proper error handling for missing env vars and SSM failures. This directly impacts cold-start latency and production reliability.
