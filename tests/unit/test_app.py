...
class TestApp:
    ...
    async def test_successfully_login(
        self, mocker, auth_service, credentials_dict, test_client
    ):
        mocker.patch('app.services.AuthService.login', return_value=None)
        response = test_client.post(f'{self.BASE_URL}/login', json=credentials_dict)
    ...
    async def test_successfully_logout(
        self, mocker, authenticated_test_client, cache_service, jwt_token
    ):
