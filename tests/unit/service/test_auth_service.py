...
class TestAuthService:
    ...
    def test_login_with_registered_audience_sets_aud_claim(
        self,
        mocker,
        auth_service: AuthService,
        user_data: dict,
        ...
        mocker.patch(
            "app.services.auth_service.UserServiceClient.get_user_by_email",
            return_value=user_data,
        )
        mocker.patch(
            "app.services.auth_service.UserServiceClient.validate_user_password",
            return_value=True,
        )
        mocker.patch(
        ...
        token, _, _, _ = auth_service.login(
            user_data["email"], "password", resource=audience_item["audience"]
        )
        decoded = JWTToken(
            **jwt.decode(
                token,
    ...
    def test_login_fails_for_unregistered_audience(
        self,
        mocker,
        auth_service: AuthService,
        user_data: dict,
        ...
        with pytest.raises(OAuthException) as excinfo:
            auth_service.login(user_data["email"], "password", resource="unknown")
    ...
    def test_client_credentials_with_registered_audience_sets_aud_claim(
        ...
        token, _, _ = auth_service.client_credentials(
            service_credential.name, password, None, resource=audience_item["audience"]
        )
        decoded = JWTToken(
            **jwt.decode(
                token,
    ...
    def test_client_credentials_fails_for_unregistered_audience(
        ...
        with pytest.raises(OAuthException) as excinfo:
            auth_service.client_credentials(
                service_credential.name, password, None, resource="unknown-audience"
            )
    ...
    def test_client_credentials_fails_for_unauthorized_client(
        ...
        with pytest.raises(OAuthException) as excinfo:
            auth_service.client_credentials(
                ...
                password,
                None,
                resource=audience_item["audience"],
            )
