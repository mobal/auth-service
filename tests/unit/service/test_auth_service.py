...
class TestAuthService:
    ...
    async def test_successfully_login(
        self, mocker, auth_service, settings, user, user_repository
    ):
        mocker.patch(self.USER_REPOSITORY_GET_BY_EMAIL, return_value=user)
    ...
    async def test_fail_to_login_due_user_not_found_by_email(
        self, mocker, auth_service, user, user_repository
    ):
        error_message = f"The requested user was not found with email={user.email}"
        mocker.patch(
            self.USER_REPOSITORY_GET_BY_EMAIL,
            side_effect=HTTPException(status.HTTP_404_NOT_FOUND, error_message),
        )
    ...
    async def test_fail_to_login_due_password_does_not_match(
        self, mocker, auth_service, password_hasher, user, user_repository
    ):
        mocker.patch(self.USER_REPOSITORY_GET_BY_EMAIL, return_value=user)
        with pytest.raises(HTTPException) as excinfo:
    ...
    async def test_successfully_logout(
        self, mocker, auth_service, cache_service, jwt_token
    ):
    ...
    async def test_fail_to_logout_due_cache_returns_false(
        self, mocker, auth_service, cache_service, jwt_token
    ):
