...
def seed_audiences() -> None:
    ...
    ``https://api.personal-backend.example`` is the resource (RFC 8707) the
    requests client-credentials-with-resource requests target; the ``user-service`` client
    is its only allowed client.
    """
    dynamodb = boto3.resource(
        "dynamodb",
        region_name=REGION,
